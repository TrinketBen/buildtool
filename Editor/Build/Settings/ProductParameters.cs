...
namespace SuperUnityBuild.BuildTool
{
    ...
    {
        public const string kEditorPrefBuildCounter = "EditorPrefBuildCounter";

        [Tooltip("Counter that is incremented whenever a build version is generated. Available for use in the version template via the $BUILD token")]
        public int buildCounter {
            get => UnityEditor.EditorPrefs.GetInt(kEditorPrefBuildCounter, 0);
            set {
                var current = buildCounter;
                if(current != value) {
                    UnityEditor.EditorPrefs.SetInt(kEditorPrefBuildCounter, value);
                }
            }
        }
